...
#### 部署架构
- 后端服务、数据库服务和后台管理系统使用Docker容器运行
- 前端服务在本地直接运行
- 通过docker-compose.yml管理容器化服务
- 支持开发模式热重载和自动重启

#### 服务组件
- Nginx：反向代理和静态资源服务（容器）
- MySQL：数据库服务（容器）
- Redis：缓存和会话存储（容器）
- Backend：Hyperf后端API服务（容器）
- Admin：Hyperf后台管理系统（容器）
- Frontend：Next.js前端开发服务（本地运行）
...
#### 热重载支持
- 前端代码修改自动刷新（本地开发服务器支持）
- 后端代码修改自动重启（Docker容器内的Hyperf支持）
- 后台管理系统代码修改自动重启（Docker容器内的Hyperf支持）
- 配置文件修改即时生效
- 数据库迁移自动执行

#### 调试支持
- 后端：Hyperf自带调试工具（容器内）
- 后台管理系统：Hyperf自带调试工具（容器内）
- 前端：本地开发工具调试支持（浏览器开发工具、Vite调试器等）
- 日志实时查看（容器日志和本地日志）
- 性能监控工具集成
- 容器与本地服务间网络调试
