...
- **评论审核机制**：用户提交评论后默认进入待审核状态，需管理员审核通过后才能在前端显示
- **评论状态管理**：支持待审核(0)、已审核通过(1)、已拒绝(2)三种状态
- **内容长度限制**：评论内容限制1000个中文字符（中文字符算1个，非中文字符算0.5个）
- **批量操作支持**：管理员可进行批量审核、批量拒绝操作
- **评论计数**：评论计数仅统计已审核通过的评论
- **管理员通知**：新评论提交后通知管理员进行审核
